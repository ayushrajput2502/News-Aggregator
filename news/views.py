...
from news.models import Headline,EHeadline,SHeadline,PHeadline,LHeadline,ENHeadline,Bookmark
...
def scrape(request):
    ...
    # d = runner.crawl(news_spider.EconomicTechSpider)
    # time.sleep(3)
    d=runner.crawl(news_spider.NewsSpider)  #tech crunch
    time.sleep(3)

    d = runner.crawl(news_spider.HindiTechnologySpider)
    time.sleep(3)
    # d = runner.crawl(news_spider.DnaTechnologySpider)
    # time.sleep(3)
...
def scrape2(request):
    ...
    time.sleep(3)
    d = runner.crawl(sports_spider.EconomicSportsSpider)
    time.sleep(3)
...
def scrape5(request):
    ...
    runner= CrawlerRunner(settings=crawler_settings)
    d=runner.crawl(entertainment_spider.DnaEntertainmentSpider)
    time.sleep(3)
    d = runner.crawl(entertainment_spider.NdtvEntertainmentSpider)
    time.sleep(3)
...
def bookmark(request):
    if request.method == 'POST':
        user = request.user
        # Get the news URL from POST data
        news_url = request.POST['news_url']
        news_title = request.POST['news_title']
        news_source = request.POST['news_source']
        news_image = request.POST['news_image']
        
        if Bookmark.objects.filter(user=user, news_url=news_url).exists():
            messages.info(request, "Selected news is already bookmarked")
            return redirect(request.META.get('HTTP_REFERER', '/'))  # Redirect to wherever you want
        else:
            # Create a bookmark entry in the database
            Bookmark.objects.create(user=user, news_url=news_url,title=news_title, image=news_image, source = news_source)
            messages.info(request, "Bookmarked successfully")
            return redirect(request.META.get('HTTP_REFERER', '/'))  # Redirect to wherever you want
...
def bookmark_view(request):
    user = request.user
    display = Bookmark.objects.filter(user=user)
    if request.method == "POST":
            news_url = request.POST['news_url']
            if Bookmark.objects.filter(user=user, news_url=news_url).exists():
                Bookmark.objects.filter(user=user, news_url=news_url).delete()
                messages.info(request, "Selected news is removed from bookmarks")
                return redirect(request.META.get('HTTP_REFERER', '/'))  # Redirect to wherever you want

    return render(request, "news/bookmarks.html",{"display":display}) 
